...
namespace 项目管理系统.仓库
{
    public partial class Frsaomachuku : Office2007Form
    {
        public Frsaomachuku()
        {
        ...
        private void Frsaomachuku_Load(object sender, EventArgs e)
        {
                ...
                if (shijichuhuo == "")
                {
                    ...
                    if (id != "")
                    {
                        ...
                        if (txtshuru.Text.Trim() == shijicaigou)
                        {
                            ...
                            string sql1 = " select 工作令号,项目名称,设备名称,编码,型号,名称,制造类型,实际采购数量,单位,采购单价,合同号,采购单价,实际到货数量,库位号,供方名称 from tb_caigouliaodan where id='" + sArray[0] + "'";
                            DataTable dt = SQLhelp.GetDataTable(sql1, CommandType.Text);
                            txtgongzuolinghao.Text = dt.Rows[0]["工作令号"].ToString();
                            txtxiangmumingcheng.Text = dt.Rows[0]["项目名称"].ToString();
                            txtshebeimingcheng.Text = dt.Rows[0]["设备名称"].ToString();
                        ...
                        if (txtshuru.Text.Trim() != shijicaigou)
                        {
                            ...
                            if (kucun >= Convert.ToDouble(txtshuru.Text.Trim()))
                            {
                                ...
                                SQLhelp.ExecuteScalar(sql, CommandType.Text);
                                string sql1 = " select 工作令号,项目名称,设备名称,编码,型号,名称,制造类型,实际采购数量,单位,采购单价,合同号,采购单价,供方名称 from tb_caigouliaodan where id='" + sArray[0] + "'";
                                DataTable dt = SQLhelp.GetDataTable(sql1, CommandType.Text);
                                txtgongzuolinghao.Text = dt.Rows[0]["工作令号"].ToString();
                                txtxiangmumingcheng.Text = dt.Rows[0]["项目名称"].ToString();
                                txtshebeimingcheng.Text = dt.Rows[0]["设备名称"].ToString();
                ...
                if (shijichuhuo == "")
                {
                    ...
                    if (id != "")
                    {
                        ...
                        if (txtshuru.Text.Trim() == shijicaigou)
                        {
                            ...
                            string sqlll = "select 料单类型 from tb_caigouliaodan where id='" + id + "'";
                            string leixing = SQLhelp.ExecuteScalar(sqlll, CommandType.Text).ToString();
                            string sql12 = "insert into tb_chuku (工作令号,项目名称,设备名称,编码,型号,名称,制造类型,实际采购数量,出库数量,出库时间,出库人,定位,单位,合同号,采购单价,总价,供方名称,料单类型) values ('" + txtgongzuolinghao.Text + "','" + txtxiangmumingcheng.Text + "','" + txtshebeimingcheng.Text + "','" + txtbianma.Text + "','" + txtxinghao.Text + "','" + txtmingcheng.Text + "','" + txtzhizaoleixing.Text + "','" + txtshijicaigoushuliang.Text + "','" + txtshuru.Text + "','" + DateTime.Now + "','" + yonghu + "','" + id + "','" + dt.Rows[0]["单位"].ToString() + "','" + txthetonghao.Text + "','" + danjia + "','" + zongjia + "','" + dt.Rows[0]["供方名称"].ToString() + "','"+leixing+"')";
                            SQLhelp.ExecuteScalar(sql12, CommandType.Text);
                            MessageBox.Show("登记成功！");
                            this.DialogResult = DialogResult.OK;
                            this.Close();

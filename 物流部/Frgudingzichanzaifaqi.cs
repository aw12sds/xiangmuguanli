...
{
    public partial class Frgudingzichanzaifaqi : DevExpress.XtraEditors.XtraForm
    {
        public Frgudingzichanzaifaqi()
        {
            InitializeComponent();
        }
        public DataTable dt;
        ...
        private void Frgudingzichanzaifaqi_Load(object sender, EventArgs e)
        {
        ...
        private void btnTijiao_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确认提交吗？", "软件提示", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (Convert.ToInt32(daohuoyanshoushuliang) <= Convert.ToInt32(shijicaigoushuliang))
                    {
                        string sql = "update tb_caigouliaodan set 本次到货验收数量='" + daohuoyanshoushuliang + "',到货验收流程状态='1待收货',提交类型='物流部再发起流程',到货类型='固定资产' where id ='" + n + "'";
                        ...
                        string sql2 = "update tb_caigouliaodan set 物流提交时间='" + tijiaoshijian + "',物流员='" + yonghu + "' where id ='" + n + "'";
                        SQLhelp.ExecuteScalar(sql2, CommandType.Text);

                        string sql3 = "insert into tb_daohuojilu (工作令号,项目名称,设备名称,名称,型号,供方名称,单位,定位,实际采购数量,到货数量,提交人,提交时间,文件定位,提交类型) values ('" + gongzuolinghao + "','" + xiangmumingcheng + "','" + shebeimingcheng + "','" + mingcheng + "','" + xinghao + "','" + gongfangmingcheng + "','" + danwei + "','" + id + "','" + shijicaigoushuliang + "','" + daohuoyanshoushuliang + "','" + yonghu + "','" + tijiaoshijian + "','" + uuid + "','物流部再发起流程')";

...
{
    public partial class Frgudingzichan : DevExpress.XtraEditors.XtraForm
    {
        public Frgudingzichan()
        {
            InitializeComponent();
        }
        ...
        private void Frgudingzichan_Load(object sender, EventArgs e)
        {
            ...
            DataTable aaaa = sqlhelp111.GetDataTable(sql1, CommandType.Text);
        ...
        private void btnTijiao_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确认提交吗？", "软件提示", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        string sql = "update tb_caigouliaodan set 本次到货验收数量='" + daohuoyanshoushuliang + "',到货检验人='" + comboBoxEdit2.Text + "',到货验收流程状态='1待收货',提交类型='物流部发起流程',到货类型='固定资产' where id ='" + n + "'";

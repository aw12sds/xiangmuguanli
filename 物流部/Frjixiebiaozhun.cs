...
{
    public partial class Frjixiebiaozhun : Office2007Form
    {
        public Frjixiebiaozhun()
        {
        ...
        DataTable dt;
        DataTable dt1;
        private void Frjixiebiaozhun_Load(object sender, EventArgs e)
        {
            Reload();
        }
        public void Reload()
        {
            ...
            if (yonghu == "尤奇" || yonghu == "庄卫星" || yonghu == "聂燕" || yonghu == "唐亚男" || yonghu == "钱陆亦")
            {

                string sql = "select 工作令号,项目名称,设备名称,项目主管,数量,采购项目负责人,时间,物流部完成时间 from tb_jishubumen where (制造类型='外购' and 是否提交=1 and 技术部通过=1 and 物流部确认=1 and 图纸下达=1 and 物流部最终确认 is null) or (制造类型='自制' and 是否提交=1 and 技术部通过=1  and 生产部确认=1 and 物流部确认=1 and 图纸下达=1 and 物流部最终确认 is null) ";
                dt = SQLhelp.GetDataTable(sql, CommandType.Text);

                string sql1 = "select 工作令号,项目名称,设备名称,项目主管,数量,采购项目负责人,时间,物流部完成时间 ,物流部最终确认时间 from tb_jishubumen where (制造类型='外购' and 是否提交=1 and 技术部通过=1 and 物流部确认=1 and 图纸下达=1 and 物流部最终确认 =1) or (制造类型='自制' and 是否提交=1 and 技术部通过=1  and 生产部确认=1 and 物流部确认=1 and 图纸下达=1 and 物流部最终确认 =1) ";
        ...
        private void dataGridViewX2_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Frjixiebiaozhunliaodan form = new Frjixiebiaozhunliaodan();
            form.gongzuolinghao = gonglinghao;
            form.xiangmumingcheng = xiangmumingcheng;

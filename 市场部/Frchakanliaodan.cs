...
{
    public partial class Frchakanliaodan : DevExpress.XtraEditors.XtraForm
    {
        public Frchakanliaodan()
        {
            InitializeComponent();
        }
        public DataTable dt;       
        ...
        private void Frchakanliaodan_Load(object sender, EventArgs e)
        {
            Reload();
        }
        public void Reload()
        {
            string sql = "select id,序号,机修件ERP,名称,型号,单位,库存数量,数量,料单类型,备注,制造类型,实际采购数量,附件名称,当前状态 from tb_caigouliaodan where 定位 = '" + id + "'and 料单类型='机修件部件清单'";
            ...
            gridView1.Columns["制造类型"].Visible = false;
        }
            ...
            if (MessageBox.Show("确认保存更改吗？", "软件提示", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                for (int i = 0; i < gridView1.RowCount; i++)
                {
                    ...
                    if (id != "")
                    {
                        ...
                        string sql2 = "update tb_caigouliaodan  set 备注='" + beizhu + "' where id ='" + id + "' ";
        ...
        private void 删除该行ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                string dangqianzhuangtai = Convert.ToString(this.gridView1.GetRowCellValue(this.gridView1.FocusedRowHandle, "当前状态"));
                if(dangqianzhuangtai!="")
                {
                    MessageBox.Show("已做工艺,无法修改！");
                    return;
                }
                    ...
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            if (shuliang11 != "")
                            {
                                ...
                                string SQL = "insert into  tb_caigouliaodan(序号,工作令号,项目名称,设备名称,型号,名称,单位,数量,备注,申购人,实际采购数量,收到料单日期,料单类型,定位)values('" + dt.Rows[i]["序号"].ToString() + "','" + gonglinghao + "','" + xiangmu + "','" + shebei + "','" + dt.Rows[i]["型号"].ToString().Trim() + "','" + dt.Rows[i]["名称"].ToString().Trim() + "','" + dt.Rows[i]["单位"].ToString() + "','" + dt.Rows[i]["数量"].ToString() + "','" + dt.Rows[i]["备注"].ToString() + "','" + yonghu + "','" + shijicaigou + "','" + DateTime.Now + "','机修件部件清单','" + id + "') ";
